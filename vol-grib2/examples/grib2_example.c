...
herr_t attr_info(hid_t loc_id, const char *name, const H5A_info_t *ainfo, void *opdata) {
    ...
    if (t_class == H5T_INTEGER) sprintf(type_name, "Integer");
    else if (t_class == H5T_FLOAT) sprintf(type_name, "Float");
    else if (t_class == H5T_STRING) sprintf(type_name, "String");
    else sprintf(type_name, "Other");
