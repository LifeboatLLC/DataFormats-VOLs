...
static int should_skip_key(const char *key)
{
    ...
    if (strcmp(key, "longitudes") == 0)
        return 1;
    if (strcmp(key, "latitudes") == 0)
        return 1;
    if (strcmp(key, "distinctLongitudes") == 0)
        return 1;
    if (strcmp(key, "distinctLatitudes") == 0)
        return 1;
    ...
    switch (codes_type) {
        ...
        case CODES_TYPE_STRING: {
            ...
            if (new_type == H5I_INVALID_HID)
                FUNC_GOTO_ERROR(H5E_DATATYPE, H5E_CANTCOPY, FAIL, "Failed to copy H5T_C_S1");
            H5Tset_strpad(new_type, H5T_STR_NULLTERM);
            break;
        }

        default:
            FUNC_GOTO_ERROR(H5E_DATATYPE, H5E_BADTYPE, FAIL, "Unsupported ecCodes type %d",
                            codes_type);
    }

    if (new_type == H5I_INVALID_HID)
        FUNC_GOTO_ERROR(H5E_DATATYPE, H5E_CANTCOPY, FAIL, "Failed to create datatype");
...
static int grib2_key_type_and_size(codes_handle *h, const char *key, int *out_type,
                                   size_t *out_size)
{
    int err;
    int t = 0;
...
static int grib2_get_key_info(codes_handle *h, const char *key, grib2_key_info_t *info)
{
...
static codes_handle *grib2_open_message_by_index(grib2_file_t *bf, size_t msg_index_1based)
{
    ...
    if (remaining > 0) {
        if (fread(buf + 16, 1, (size_t) remaining, bf->grib2) != (size_t) remaining) {
            fprintf(stderr,
                    "Failed to read full GRIB2 message (len=%" PRIu64 ") at offset %" PRIu64 "\n",
                    msg_len, off);
            free(buf);
            return NULL;
        }
...
static herr_t grib2_read_data(grib2_dataset_t *dset)
{
    ...
    if (!tmp)
        FUNC_GOTO_ERROR(H5E_VOL, H5E_CANTALLOC, FAIL,
                        "Failed to allocate buffer for double values buffer");
...
herr_t grib2_file_get(void *file, H5VL_file_get_args_t *args, hid_t __attribute__((unused)) dxpl_id,
                       ...
                       hid_t __attribute__((unused)) dxpl_id, void __attribute__((unused)) * *req)
{
    ...
    if (!h) {
        FUNC_GOTO_ERROR(H5E_VOL, H5E_BADVALUE, NULL, "Failed to get GRIB2 message handle ");
    }
...
static int check_grib2_dataset_name(const char *key_view)
{
    ...
    if (n == 3) {
        return (strcmp(key_view, "lon") == 0 || strcmp(key_view, "lat") == 0);
    }
    if (n == 6) {
                         ...
                         hid_t __attribute__((unused)) dxpl_id, void __attribute__((unused)) * *req)
{
    ...
    if ((o->obj_type != H5I_FILE) && (o->obj_type != H5I_GROUP))
        FUNC_GOTO_ERROR(H5E_VOL, H5E_BADVALUE, NULL,
                        "Invalid GRIB2 object identifieri, should be file or group");
    ...
    dset->data = NULL;
    dset->data_size =
        0; /* The value is set to the size of the data buffer; see grib2_read_data function below */
    ...
    if (o->obj_type == H5I_FILE) {
        /* If we are passed file object, then the name of the dataset should be in the form
         * /message_<N>/<key> */
        ...
        if (check_grib2_dataset_name(key_view) == 0) {
            FUNC_GOTO_ERROR(H5E_VOL, H5E_BADVALUE, NULL,
                            "Invalid dataset name, expect lon, lat or values strings");
        }
...
static herr_t grib2_read_attr_data(grib2_attr_t *attr)
{
    ...
    if (attr->codes_type == CODES_TYPE_STRING) {
        ...
        if (!tmp)
            FUNC_GOTO_ERROR(H5E_VOL, H5E_CANTALLOC, FAIL,
                            "Failed to allocate buffer for string key");
                      ...
                      void __attribute__((unused)) * *req)
{
    ...
    if (grib2_get_key_info(parent_obj->u.group.msg->h, key_view, &ki) != 0) {
        FUNC_GOTO_ERROR(H5E_ATTR, H5E_BADVALUE, NULL, "Failed to discover key info");
    }
    ...
    if (message_type == CODES_TYPE_STRING) {
        ...
        attr->nvals = ki.nelems;
        if (message_type == CODES_TYPE_LONG)
            attr->data_size = len * sizeof(long);
        if (message_type == CODES_TYPE_DOUBLE)
            attr->data_size = len * sizeof(double);
        if (message_type == CODES_TYPE_BYTES)
            attr->data_size = len * sizeof(unsigned char);
    }

    /* Find corresponding HDF5 native type; set a special flag if GRIB2 type is string */
    attr->codes_type = message_type;
    if (grib2_get_hdf5_type(attr->codes_type, &attr->type_id,
                            (ki.native_type == CODES_TYPE_STRING) ? ki.str_len : ki.nelems) < 0) {
        FUNC_GOTO_ERROR(H5E_ATTR, H5E_BADVALUE, NULL,
    ...
    /* By now we know actual size of the string that ECCODES will read , set appropriate fixed
     * string length */
    if (message_type == CODES_TYPE_STRING) {
...
herr_t grib2_attr_close(void *attr, hid_t dxpl_id, void **req)
{
    ...
    if (o->ref_count == 0) {
        ...
        /* TODO: We cannot delete it handle like below, since it will cause issues when closeing the
           group. Check that we shouldn't delete handle here  */
        if (a->msg) {
            if (a->msg->h) {
...
// cppcheck-suppress constParameterCallback
herr_t grib2_attr_get(void *obj, H5VL_attr_get_args_t *args, hid_t __attribute__((unused)) dxpl_id,
                      void __attribute__((unused)) * *req)
{
                           ...
                           void __attribute__((unused)) * *req)
{
    ...
    if (!obj || !loc_params || !args)
        FUNC_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL,
                        "Invalid arguments to grib2_attr_specific function");
    ...
    switch (args->op_type) {
        ...
        case H5VL_ATTR_ITER: {
            ...
            if (iter_args->op) {
                unsigned long flags = 0;
                /* TODO: we should allow to set the flags through environement variable or vol
                 * info*/
                flags |= CODES_KEYS_ITERATOR_SKIP_DUPLICATES;
                /*
                flags |= CODES_KEYS_ITERATOR_SKIP_COMPUTED;
                ...
                */
                ...
                if (!keys_tmp)
                    FUNC_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                                    "cannot allocate temp key array");
                ...
                while (codes_keys_iterator_next(it)) {
                    ...
                    if (iter_index == num_attrs)
                        FUNC_GOTO_ERROR(H5E_ATTR, H5E_BADVALUE, FAIL,
                                        "Iteration index is out of bounds");
                ...
                for (hsize_t i = 0; i < keys_count; i++) {
                    ...
                    if (ki.native_type == CODES_TYPE_LONG)
                        key_size = ki.nelems * sizeof(long);
                    if (ki.native_type == CODES_TYPE_DOUBLE)
                        key_size = ki.nelems * sizeof(double);
                    if (ki.native_type == CODES_TYPE_STRING)
                        key_size = ki.nelems * ki.str_len; /* includes NUL */
                    if (ki.native_type == CODES_TYPE_BYTES)
                        key_size = ki.nelems;
                    ...
                    /* herr_t cb_ret = iter_args->op( 0, keys_tmp[i], &attr_info,
                     * iter_args->op_data); */
                    herr_t cb_ret =
                        iter_args->op(H5I_INVALID_HID, keys_tmp[i], &attr_info, iter_args->op_data);
...
herr_t grib2_init_connector(hid_t __attribute__((unused)) vipl_id)
{
    ...
    if ((H5_grib2_err_class_g = H5Eregister_class(
