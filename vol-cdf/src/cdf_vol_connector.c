...
static herr_t cdf_get_hdf5_type_from_cdf(long cdf_datatype, hid_t *type_id)
{
    ...
    switch (cdf_datatype) {
        ...
        case CDF_EPOCH16: /* 16-byte, two 8-byte floating point values representing picoseconds
                             since 0000-01-01 */
        {
        ...
        default:
            FUNC_GOTO_ERROR(H5E_DATATYPE, H5E_UNSUPPORTED, FAIL, "Unsupported CDF datatype: %ld",
                            cdf_datatype);
    }
...
static herr_t prepare_converted_buffer(hid_t type_id, hid_t mem_type_id, size_t num_elements,
                                       void *source_buf, size_t source_size, void **out_buffer,
                                       size_t *out_buffer_size, hbool_t *out_tconv_buf_allocated)
{
    ...
    if (!source_buf || !source_size || !out_buffer || !out_buffer_size ||
        !out_tconv_buf_allocated) {
...
/* Helper function that loops through all attributes in the CDF file and gathers basic information
 * about each one. The information gathered includes*/
static herr_t cache_cdf_attribute_info(cdf_file_t *file)
{
    ...
    file->gAttrCache = calloc(file->numgAttrs, sizeof(cdf_attr_cache_t));
    file->vAttrCache = calloc(file->numvAttrs, sizeof(cdf_attr_cache_t));
    if (file->gAttrCache == NULL || file->vAttrCache == NULL) {
        FUNC_GOTO_ERROR(H5E_FILE, H5E_CANTALLOC, FAIL,
                        "Failed to allocate memory for CDF attribute caches");
    }
    ...
    for (long attrNum = 0; attrNum < file->numAttrs; attrNum++) {
        ...
        if (attrScope == GLOBAL_SCOPE) {
            ...
            if (status != CDF_OK) {
                cdf_print_error(status);
                FUNC_GOTO_ERROR(H5E_FILE, H5E_CANTGET, FAIL,
                                "failed to get number of entries for vAttribute #%ld", attrNum);
            }

            vAttr_count++; /* Increment vAttrCache index */

        } else { /* Sanity check */
            FUNC_GOTO_ERROR(H5E_FILE, H5E_UNSUPPORTED, FAIL,
                            "Attribute scope returned by CDFinquireAttr is not valid");
        }
    }
                    ...
                    hid_t __attribute__((unused)) dxpl_id, void __attribute__((unused)) * *req)
{
    ...
    if (file->majority != ROW_MAJOR) {
        FUNC_GOTO_ERROR(H5E_FILE, H5E_UNSUPPORTED, NULL, "Only ROW_MAJOR CDF files are supported\n");
    }
                      ...
                      void __attribute__((unused)) * *req)
{
    ...
    if (o->ref_count == 0) {
        ...
        if (o->u.file.gAttrCache) {
            for (long i = 0; i < o->u.file.numgAttrs; i++) {
                free(o->u.file.gAttrCache[i].attrName);
            }
        ...
        if (o->u.file.vAttrCache) {
            ...
            free(o->u.file.vAttrCache);
            o->u.file.vAttrCache = NULL;
        }
                       ...
                       hid_t __attribute__((unused)) dxpl_id, void __attribute__((unused)) * *req)
{
    ...
    if ((dset->name = strdup(resolved_name)) == NULL) {
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, NULL, "Failed to duplicate dataset name string");
    }
    ...
    if (dset->var_num < CDF_OK) {
        ...
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTGET, NULL, "Failed to get variable number from name: %s",
                        dset->name);
    }

    char tmp[CDF_VAR_NAME_LEN256 + 1]; /* Dummy buffer - we already know the name */
    /* Get variable info. */
    status = CDFinquirezVar(file->id, dset->var_num, tmp, &data_type, &dset->num_elements,
                            &dset->num_dims, dset->dim_sizes, &dset->rec_vary, dset->dim_varys);
    if (status != CDF_OK) {
        cdf_print_error(status);
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTGET, NULL, "Failed to get variable info for var num %ld",
                        dset->var_num);
    }

    if (data_type == CDF_CHAR || data_type == CDF_UCHAR) {
        /* CDF chars are exposed as null-terminated strings even if the underlying CDF attribute is
         * not null-terminated */
        dset->type_id =
        ...
        if (cdf_get_hdf5_type_from_cdf(data_type, &dset->type_id) < 0) {
            FUNC_GOTO_ERROR(H5E_DATATYPE, H5E_CANTGET, NULL,
                            "Failed to map CDF data type %ld to HDF5 datatype", data_type);
        }
    }
    ...
    if (status != CDF_OK) {
        cdf_print_error(status);
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTGET, NULL, "Failed to get number of records in CDF variable");
    }
                        ...
                        void __attribute__((unused)) * *req)
{
    ...
    if (file_space_id[0] == 0) {
        ...
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "failed to get file dataspace selection type");
    }
    ...
    if ((cdf_buf = malloc(cdf_data_size)) == NULL) {
        FUNC_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                        "failed to allocate buffer for CDF data");
    }
    ...
    status = CDFhyperGetzVarData(id, d->var_num, 0L, /* start reading from first record */
                                 d->num_records,     /* number of records to read (ALL) */
                                 1L,                 /* stride */
                                 indices, counts, intervals, cdf_buf);
    if (status != CDF_OK) {
        cdf_print_error(status);
        FUNC_GOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL, "failed to read data from CDF variable '%s'",
                        d->name);
    }
    ...
    if (file_sel_type != H5S_SEL_ALL && effective_file_space_id != d->space_id) {
        ...
        if (num_elements < 0) {
            FUNC_GOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "invalid number of elements for gather");
        }
...
herr_t cdf_dataset_close(void *dset, hid_t dxpl_id, void **req)
{
    ...
    if (!d->parent_file) {
        FUNC_DONE_ERROR(H5E_DATASET, H5E_BADVALUE, FAIL, "Dataset has no valid parent file reference");
    }

    /* Decrement dataset's ref count */
    if (d->ref_count == 0) {
        FUNC_DONE_ERROR(H5E_DATASET, H5E_CANTCLOSEOBJ, FAIL, "Dataset already closed (ref_count is 0)");
    }
    d->ref_count--;
    ...
    if (d->ref_count == 0) {
        ...
        if (cdf_file_close(d->parent_file, dxpl_id, req) < 0) {
            FUNC_DONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, FAIL, "Failed to close dataset file object");
        }
...
herr_t cdf_group_get(void *obj, H5VL_group_get_args_t *args, hid_t __attribute__((unused)) dxpl_id,
                     void __attribute__((unused)) * *req)
{
...
static size_t calculate_gEntry_str_len(long cdf_datatype, long num_elements)
{
    ...
    switch (cdf_datatype) {
        ...
        case CDF_REAL4:
        case CDF_FLOAT:
            value_size = FLT_DECIMAL_DIG; /* Maximum precision length needed for conversion of float
                                             to string and back */
            break;
        ...
        case CDF_DOUBLE:
        case CDF_EPOCH:
            value_size = DBL_DECIMAL_DIG; /* Maximum precision length needed for conversion of
                                             double to string and back */
            break;

        case CDF_EPOCH16:
            value_size =
                2 * DBL_DECIMAL_DIG + 2 + 2; /* 2 doubles + a comma and a space + 2 brackets */
            break;
                    ...
                    void __attribute__((unused)) * *req)
{
    ...
    if (attr->attrNum >= CDF_OK) {
        ...
        if (!base_name && attr->indexed) {
            FUNC_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, NULL, "Attribute base_name is NULL after parsing");
        }
        ...
        if (!attr->name) {
            ...
            FUNC_GOTO_ERROR(H5E_ATTR, H5E_NOTFOUND, NULL,
                            "Attribute '%s' not found in CDF file.", name);
        }
    }
    ...
    if (status != CDF_OK) {
        cdf_print_error(status);
        FUNC_GOTO_ERROR(H5E_ATTR, H5E_READERROR, NULL, "failed to read data from CDF attribute '%s'",
                        attr->name);
    }
    ...
    if (is_file_or_group && attr->scope == GLOBAL_SCOPE) {
        ...
        if (attr->indexed) {
            ...
            if (attr->datatype == CDF_CHAR || attr->datatype == CDF_UCHAR) { /* String attribute */
                ...
                /* CDF chars are exposed as null-terminated strings even if the underlying CDF
                 * attribute is not null-terminated */
                attr->type_id =
            ...
            /* Allocate array to store indices of existing gEntries (those that can be read
             * successfully) */
    ...
    /* Only allow vAttributes to be opened when using dataset obj_type */
        ...
        if (parent_dset->var_num > max_zEntry) {
            FUNC_GOTO_ERROR(H5E_ATTR, H5E_NOTFOUND, NULL,
                            "vAttribute '%s' does not exist for variable number %ld", attr->name,
                            parent_dset->var_num);
        }
        ...
        /* Create dataspace dynamically based on num_elements and whether or not the attribute is a
         * string */
        if (attr->datatype == CDF_CHAR || attr->datatype == CDF_UCHAR) { /* String attribute */
            /* Create scalar dataspace for string attribute */
            if ((attr->space_id = H5Screate(H5S_SCALAR)) < 0) {
            ...
            /* CDF chars are exposed as null-terminated strings even if the underlying CDF attribute
...
static herr_t stringify_single_gEntry_value(long cdf_datatype, unsigned char *buffer, char *out,
                                            size_t size)
{
    herr_t ret_value = SUCCEED;
    ...
    switch (cdf_datatype) {
        ...
        default:
            ...
            FUNC_GOTO_ERROR(H5E_VOL, H5E_BADVALUE, FAIL,
                            "Unsupported CDF datatype for stringification: %ld", cdf_datatype);
            break;
    }
...
static herr_t get_cdf_datatype_name(long cdf_datatype, char *name_buf, size_t buf_size)
{
    ...
    switch (cdf_datatype) {
        ...
        case CDF_REAL8:
            ...
            break;
        case CDF_DOUBLE:
            type_name = "CDF_DOUBLE";
            break;
        case CDF_EPOCH:
            type_name = "CDF_EPOCH";
            break;
        case CDF_EPOCH16:
            type_name = "CDF_EPOCH16";
            break;
        default:
            FUNC_GOTO_ERROR(H5E_VOL, H5E_BADVALUE, FAIL, "Unsupported CDF datatype: %ld",
                            cdf_datatype);
            break;
    }
                                ...
                                long num_elements, size_t str_size, char *out)
{
    ...
    if (!out || str_size == 0) {
        FUNC_GOTO_ERROR(H5E_VOL, H5E_BADVALUE, FAIL,
                        "Invalid output buffer or str_size for gEntry stringification");
    }
    ...
    if (get_cdf_datatype_name(cdf_datatype, cdf_type_name, sizeof(cdf_type_name)) < 0) {
        FUNC_GOTO_ERROR(H5E_VOL, H5E_BADVALUE, FAIL,
                        "Failed to get CDF datatype name for gEntry stringification");
    }

    /* Format the prefix: "<index> (CDF_DATATYPE/num_elements): "*/
    int prefix_len =
        snprintf(out, str_size, "%ld (%s/%ld): ", gEntry_index, cdf_type_name, num_elements);
    if (prefix_len < 0 || (size_t) prefix_len >= str_size) {
        FUNC_GOTO_ERROR(
            H5E_VOL, H5E_BADVALUE, FAIL,
    ...
    for (size_t i = 0; i < (size_t) num_elements; i++) {
        ...
        ret_value = stringify_single_gEntry_value(
            cdf_datatype, buffer + (i * elem_size), /* Pointer to current element */
            tmp, sizeof(tmp));
        if (ret_value != SUCCEED) {
            FUNC_GOTO_ERROR(H5E_VOL, H5E_BADVALUE, FAIL, "Failed to stringify element %zu", i);
        }
    ...
    if (a->scope == GLOBAL_SCOPE) {
        if (a->indexed) {
            ...
            if (a->datatype == CDF_CHAR || a->datatype == CDF_UCHAR) {
                ...
                str_buf = (char *) malloc(a->num_elements); /* Temporary buffer for CDF string (can
                                                               be big so avoid stack allocation) */
                if (!str_buf) {
                    FUNC_GOTO_ERROR(
                        H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                        "Failed to allocate temporary buffer for gAttribute string read");
                }

                memset(str_buf, 0, a->num_elements);

                status = CDFgetAttrgEntry(id, a->attrNum, a->index, str_buf);
                if (status != CDF_OK) {
                    cdf_print_error(status);
                    FUNC_GOTO_ERROR(
                ...
                /* Free temporary CDF buffer if new buffer was allocated in
                 * prepare_converted_buffer() */
                if (tconv_buf_allocated && cdf_buf) {
                    free(cdf_buf);
                    cdf_buf = NULL;
                }
        ...
        if (a->datatype == CDF_CHAR || a->datatype == CDF_UCHAR) {
            ...
            str_buf = (char *) malloc(a->num_elements); /* Temporary buffer for CDF string (can be
                                                           big so avoid stack allocation) */
            if (!str_buf) {
                FUNC_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
...
herr_t cdf_attr_get(void *obj, H5VL_attr_get_args_t *args, hid_t __attribute__((unused)) dxpl_id,
                    void __attribute__((unused)) * *req)
{
...
herr_t cdf_attr_close(void *attr, hid_t dxpl_id, void **req)
{
    ...
    if (o->ref_count == 0) {
        ...
        if (a->space_id != H5I_INVALID_HID && H5Sclose(a->space_id) < 0) {
            FUNC_DONE_ERROR(H5E_ATTR, H5E_CLOSEERROR, FAIL, "Failed to close attribute dataspace");
        }
        /* Attempt to close type_id. */
        if (a->type_id != H5I_INVALID_HID && H5Tclose(a->type_id) < 0) {
            FUNC_DONE_ERROR(H5E_ATTR, H5E_CLOSEERROR, FAIL, "Failed to close attribute datatype");
        }
                         ...
                         void __attribute__((unused)) * *req)
{
    ...
    cdf_object_t *file_obj = o->parent_file;
    if (!file_obj || file_obj->obj_type != H5I_FILE) {
        FUNC_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "Invalid file object in attr_specific");
    }
    file = &file_obj->u.file;
    CDFid id = file->id;
    ...
    switch (args->op_type) {
        case H5VL_ATTR_EXISTS: {
            ...
            *args->args.exists.exists = false; /* Assume it doesn't exist by default */
            ...
            if (!base_name && indexed) {
                FUNC_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, FAIL, "Attribute base_name is NULL after parsing");
            }
            ...
            if (o->obj_type == H5I_FILE || o->obj_type == H5I_GROUP) {
                ...
                for (long i = 0; i < file->numgAttrs; i++) {
                    ...
                    /* If exact requested name doesn't exist as attribute, check for '_#' suffix case */
                    if (indexed && strcmp(file->gAttrCache[i].attrName, base_name) == 0) {
                        ...
                        if (status == CDF_OK) {
                            *args->args.exists.exists = true;
                            ...
                            FUNC_GOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL, "Failed gEntry existence check");
                        }
                    }
                }

            } else if (o->obj_type == H5I_DATASET) {
                /* Get parent dataset object */
                cdf_dataset_t *dset = &o->u.dataset;

                /* Search vAttrCache for matching name */
        ...
        case H5VL_ATTR_ITER: {
            ...
            long datatype, num_elements; /* For CDFinquireAttr() */
            hsize_t data_size;           /* Expected size of attribute data */
            ...
            if (o->obj_type == H5I_FILE || o->obj_type == H5I_GROUP) {
                ...
                if (*iter_args->idx >= (hsize_t) file->numgAttrs) {
                ...
                if (iter_args->op) {
                    for (long cache_index = (long) (*iter_args->idx); cache_index < file->numgAttrs;
                        ...
                        /* Grab pre-cached info for this specific gAttribute */
                ...
                for (long cache_index = 0; cache_index < file->numvAttrs; cache_index++) {
                ...
                if (iter_args->op) {
                    for (long i = (long) (*iter_args->idx); i < varAttrCount; i++) {
                        ...
                        /* Grab the index of the current attribute */
                        long index = varAttrCacheIndices[i];
                        ...
                        /* Calculate data size of attribute based on data type */
                        if (datatype == CDF_CHAR || datatype == CDF_UCHAR) {
                            data_size = num_elements + 1; /* +1 for null terminator */
                        } else {
                            /* Get HDF5 datatype */
                            hid_t type_id = H5I_INVALID_HID;
                            if (cdf_get_hdf5_type_from_cdf(datatype, &type_id) < 0) {
                                FUNC_GOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                         ...
                         void __attribute__((unused)) * *req)
{
    ...
    if (o->obj_type != H5I_FILE && o->obj_type != H5I_GROUP) {
        FUNC_GOTO_ERROR(H5E_ARGS, H5E_BADVALUE, FAIL, "Object type not supported");
    }
    ...
    switch (args->op_type) {
        case H5VL_LINK_EXISTS: {
            ...
            /* Check that variable with the provided name exists*/
            status = CDFconfirmzVarExistence(file_obj->u.file.id, resolved_name);

            if (status == CDF_OK) {
                *args->args.exists.exists = 1;
            } else if (status == NO_SUCH_VAR) {
