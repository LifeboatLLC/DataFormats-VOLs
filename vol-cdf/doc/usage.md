...
# NOTE that the nproc command doesn't exist in macOS. 
# You may create an alias for nprox on macOS systems with `alias nproc="sysctl -n hw.logicalcpu"` (put in shell rc file)
```
...
- **`CDF_VOL_BUILD_EXAMPLES`** (default: `ON`): Build example programs

  To disable building examples:
  ```bash
  cmake .. -DCDF_VOL_BUILD_EXAMPLES=OFF
  ```
...
### Example Programs
This distribution contains an example program `cdf_example.c`. 
...
Usage:
```bash
./cdf_example # Make sure working directory is `vol-cdf/build/examples`
```

`cdf_example.c` program demonstrates functionality of the CDF VOL connector. 

The program 
 - registers CDF VOL connector and creates file access property list to use the connector
 - open CDF file using the property
 - finds the number of global attributes (file-level metadata attributes) using group iteration
 - opens the first global attribute (without index, so an array of stringified gEntries is returned)
 - opens the gEntry at index #1 and #3 of the first global attribute (Raw gEntry values)
 - finds the number of datasets and their names using dataset iteration
 - finds the number of vAttributes attached to each dataset (using attribute iteration INSIDE of the dataset iteration callback)
 - reads a dataset and prints its data
 - prints all names of all datasets and all attributes (global and variable attributes)
