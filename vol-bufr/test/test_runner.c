/*
 * Test program for BUFR VOL connector
 * Tests reading a BUFR file through HDF5 interface
 */
...
int main(int argc, char **argv)
{
    ...
    /* Run BUFR functionality tests (each test manages its own connector registration) */
    ...
    if (run_all || strcmp(test_name, "dataset_open_close") == 0)
        num_failures += (OpenBUFRDatasetTest("temp.bufr", "pressure") != 0 ? 1 : 0);
