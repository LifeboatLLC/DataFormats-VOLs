...
herr_t test_registration_by_value(void)
{
    ...
    if ((is_registered = H5VLis_connector_registered_by_name(BUFR_VOL_CONNECTOR_NAME)) < 0)
        TEST_ERROR;
    if (true == is_registered) {
        if ((pre_id = H5VLget_connector_id_by_name(BUFR_VOL_CONNECTOR_NAME)) < 0)
            TEST_ERROR;
    ...
    if ((vol_id = H5VLregister_connector_by_value(BUFR_VOL_CONNECTOR_VALUE, H5P_DEFAULT)) < 0)
        TEST_ERROR;
...
herr_t test_getters(void)
{
    ...
    if ((vol_id_out = H5VLget_connector_id_by_name(BUFR_VOL_CONNECTOR_NAME)) < 0)
        TEST_ERROR;
